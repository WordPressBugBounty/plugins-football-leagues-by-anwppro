...
?>
<div class="anwp-b-wrap competition competition-tabs competition__inner competition-<?php echo (int) $competition_post_id; ?> competition-mixed">
	...
	if ( apply_filters( 'anwpfl/tmpl-competition/render_header', true, $competition_post_id ) ) {
		...
		$competition_shortcode_attr = [
			...
			'season_selector' => 1,
			'title_field'     => anwp_football_leagues()->customizer->get_value( 'competition', 'competition_title_field' ),
		];

		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo anwp_football_leagues()->template->shortcode_loader( 'competition_header', $competition_shortcode_attr );
	}
			...
			<?php foreach ( $competitions as $ii => $competition ) : ?>
				<div class="anwp-fl-tab__content" id="anwp_c_id_<?php echo (int) $competition->ID; ?>">
					...
					*/
					...
					if ( 'desc' === anwp_football_leagues()->customizer->get_value( 'competition', 'competition_rounds_order' ) ) {
					...
					if ( count( $rounds ) > 1 ) :
						?>
						<div class="anwp-fl-tabs mb-3 d-flex flex-wrap competition__tabs" id="anwp-rounds-tabs-c<?php echo (int) $ii; ?>" role="tablist">
					...
					<div class="anwp-fl-tabs-content competition-round__tabs-content">
						<?php foreach ( $rounds as $rr => $round ) : ?>
							<div class="anwp-fl-tab__content <?php echo ( count( $rounds ) > 1 ) ? 'd-none' : ''; ?>" id="anwp_c_id_<?php echo (int) $competition->ID; ?>_round_id_<?php echo (int) $round->id; ?>">
								<?php
											...
											if ( apply_filters( 'anwpfl/tmpl-competition/render_group_title', $render_group_title, $group, $competition, $competition_post_id ) ) :
												...
												<div class="competition__group-title">
													...
													anwp_football_leagues()->load_partial(
														[
															'text'  => $group->title,
															'class' => 'competition-tabs__group-title',
														],
														'general/header'
													);
													?>
											...
											foreach ( $matches as $match ) :
												...
												if ( 'round-robin' === $competition_type && $match_week !== (int) $match->match_week && (int) $match->match_week ) :
													...
													<div class="anwp-section">
														...
														anwp_football_leagues()->load_partial(
															[
																'text'  => anwp_football_leagues()->options->get_text_matchweek( $match->match_week ),
																'class' => 'competition__round-title competition-tabs__round-title',
															],
															'general/subheader'
														);
														?>
