...
$club_obj  = anwp_fl()->club->get_club( $data['club_id'] );

if ( empty( $club_obj->logo ) ) {
	if ( empty( $club_obj ) ) {
		$club_obj = (object) [
			'title' => '',
			'link'  => '',
...
<div class="club-title py-2 px-1 px-sm-2 d-flex align-items-center anwp-bg-light position-relative <?php echo esc_attr( $is_home ? '' : 'flex-row-reverse' ); ?> <?php echo esc_attr( $data['class'] ); ?>">
	<img loading="lazy" width="35" height="35" class="anwp-object-contain club-title__logo anwp-flex-none mb-0 anwp-w-35 anwp-h-35"
		src="<?php echo esc_url( $club_obj->logo ); ?>" alt="<?php echo esc_attr( $club_obj->title ); ?>">
	<div class="club-title__name anwp-text-xl <?php echo esc_attr( $is_home ? 'ml-3 mr-auto' : 'mr-3 ml-auto' ); ?> <?php echo esc_attr( $data['class_title'] ); ?>">
