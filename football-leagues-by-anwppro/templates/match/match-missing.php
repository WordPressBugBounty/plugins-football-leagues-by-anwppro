...
$missing_players = anwp_fl()->match->get_game_missed_players( $data['match_id'] );
...
// Prepare squad
$home_squad = anwp_fl()->club->tmpl_prepare_club_squad( $data['home_club'], $data['season_id'] );
$away_squad = anwp_fl()->club->tmpl_prepare_club_squad( $data['away_club'], $data['season_id'] );

$positions = anwp_fl()->data->get_positions_l10n();

$positions_l10n = [
	'g' => anwp_fl()->get_option_value( 'text_abbr_goalkeeper' ) ?: $positions['g'],
	'd' => anwp_fl()->get_option_value( 'text_abbr_defender' ) ?: $positions['d'],
	'm' => anwp_fl()->get_option_value( 'text_abbr_midfielder' ) ?: $positions['m'],
	'f' => anwp_fl()->get_option_value( 'text_abbr_forward' ) ?: $positions['f'],
];

$data['players'] += anwp_fl()->player->get_players_by_ids( wp_list_pluck( $missing_players, 'player' ) );
		...
		<div class="anwp-flex-1 pr-sm-3 match-missing__team-wrapper">
			...
			*/
			foreach ( $missing_players as $missing_player ) :
				...
				$player    = $data['players'][ $player_id ] ?? false;
