...
class AnWPFL_Health {
	...
	private function test_health_main_stage_id() {

		global $wpdb;

		$incorrect_games = absint( $wpdb->get_var( "SELECT COUNT(*) FROM $wpdb->anwpfl_matches WHERE main_stage_id = 0;" ) ?: 0 );
		...
		*/
		...
		return rest_ensure_response(
			[
				'status'   => $incorrect_games ? 'problems' : 'ok',
				'fix_data' => [
					'text'      => $output_text,
					'link_type' => 'action',
					'links'     => $fix_links,
	...
	*/
	...
	/**
	 * Try to fix Games with incorrect Main Stage ID.
	 *
	 * @return bool|WP_Error|WP_HTTP_Response|WP_REST_Response
	 */
	private function test_fix_main_stage_id() {
		global $wpdb;
		...
		return $this->test_health_main_stage_id();
	}
}
