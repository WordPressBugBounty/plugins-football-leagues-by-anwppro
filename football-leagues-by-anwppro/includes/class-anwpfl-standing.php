...
class AnWPFL_Standing extends CPT_Core {
	 ...
	 * @return void
	 */
	public function calculate_standing_prepare( int $match_id, int $competition_id = 0, int $group_id = 0 ) {
			...
			<?php if ( $fixed ) : ?>
				...
				<?php else : ?>
					...
					<script type="text/javascript">
						...
						window._AnWP_FL_Standing_Data = <?php echo wp_json_encode( $standing_data ); ?>;
					</script>
	 ...
	 */
	public function get_games_with_custom_outcome( int $competition_id, int $group_id ): array {
		global $wpdb;
		...
		INNER JOIN $wpdb->anwpfl_matches m ON m.match_id = p.post_id
		WHERE p.meta_key = '_anwpfl_custom_outcome' AND p.meta_value = 'yes' AND m.finished = 1
		";

		$custom_query .= $wpdb->prepare( ' AND m.competition_id = %d ', $competition_id );
		$custom_query .= $wpdb->prepare( ' AND m.group_id = %d ', $group_id );
