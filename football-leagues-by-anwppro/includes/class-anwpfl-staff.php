...
class AnWPFL_Staff extends CPT_Core {
	 ...
	 */
	public function get_staff_list(): array {
	...
	public function get_staff_id_by_external_id( $external_id ) {
		...
		return $wpdb->get_var(
			$wpdb->prepare(
				...
				SELECT post_id
				FROM $wpdb->postmeta
				WHERE meta_key = '_anwpfl_staff_external_id' AND meta_value = %s
				",
				$external_id
			)
		);
		...
		if ( null === $output ) {
			...
			$rows = $wpdb->get_results(
				"
					...
					FROM $wpdb->postmeta
					WHERE meta_key = '_anwpfl_job_title' AND meta_value != ''
				"
			);
	...
	public function get_staff_photo_map() {
		...
		if ( null === $output ) {
			...
			$rows = $wpdb->get_results(
				...
				WHERE p.post_status = 'publish' AND p.post_type = 'anwp_staff' AND pm1.meta_value != ''
				"
			);
