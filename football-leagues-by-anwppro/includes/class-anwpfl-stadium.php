...
class AnWPFL_Stadium extends CPT_Core {
	 ...
	 * @return array
	 */
	private function get_all_stadiums() {

		$cache_key = 'FL-STADIUMS-LIST';
		...
		$all_stadiums = $wpdb->get_results(
			"
			SELECT p.ID id, p.post_title pt,
				MAX( CASE WHEN pm.meta_key = '_anwpfl_city' THEN pm.meta_value ELSE '' END) as city
			FROM $wpdb->posts p
			LEFT JOIN $wpdb->postmeta pm ON ( pm.post_id = p.ID )
			WHERE p.post_status = 'publish' AND p.post_type = 'anwp_stadium'
			GROUP BY p.ID
			ORDER BY p.post_title
			"
		);

		if ( empty( $all_stadiums ) ) {
			return [];
		}

		anwp_football_leagues()->cache->set( $cache_key, $all_stadiums );
	...
	public function get_stadiums() {
		...
		$all_stadiums = $this->get_all_stadiums();
		...
		foreach ( $all_stadiums as $stadium ) {
			...
			$stadium_title = $stadium->pt;

			if ( $stadium->city ) {
				$stadium_title .= ' (' . $stadium->city . ')';
			}

			$output_data[] = (object) [
				'id'    => absint( $stadium->id ),
				'title' => $stadium_title,
			];
		}
	...
	public function sortable_columns( $sortable_columns ) {
		...
		return array_merge( $sortable_columns, [ 'stadium_id' => 'ID' ] );
	}
	...
	public function columns( $columns ) {
		...
		$new_columns = [
			...
			'stadium_id'           => esc_html__( 'ID', 'anwp-football-leagues' ),
			'anwpfl_stadium_photo' => esc_html__( 'Photo', 'anwp-football-leagues' ),
		];
		...
		$new_columns_order = [ 'cb', 'title', 'anwpfl_stadium_photo', 'anwpfl_city', 'comments', 'date', 'stadium_id' ];
		$new_columns       = [];
	...
	public function columns_display( $column, $post_id ) {
		switch ( $column ) {
			...
			case 'anwpfl_stadium_photo':
				$logo = get_post_meta( $post_id, '_anwpfl_photo', true );
	 ...
	 * @since 0.14.0
	 */
	public function get_stadium_id_by_external_id( $external_id ) {
		...
		return $wpdb->get_var(
			$wpdb->prepare(
				...
				SELECT post_id
				FROM $wpdb->postmeta
				WHERE meta_key = '_anwpfl_stadium_external_id' AND meta_value = %s
				",
				$external_id
			)
		);
