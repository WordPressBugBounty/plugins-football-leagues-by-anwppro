...
class AnWPFL_Data {
	...
	public function get_initial_countries() {
		...
		return array_merge(
			...
			[
				...
				'yu'                => esc_html_x( 'Yugoslavia', 'country', 'anwp-football-leagues' ),
				'su'                => esc_html_x( 'Soviet Union', 'country', 'anwp-football-leagues' ),
			]
		);
	}
		...
		if ( ! empty( $custom_countries_raw ) && is_array( $custom_countries_raw ) ) {
			foreach ( $custom_countries_raw as $custom_country ) {
				if ( ! empty( $custom_country['title'] ) ) {
					$this->countries[ '___' . $custom_country['title'] ] = $custom_country['title'];
				}
			}
		}
	...
	public function get_custom_county_data( $country_code ) {
		...
		return isset( $custom_countries[ $country_code ] ) ? $custom_countries[ $country_code ] : false;
	}
	...
	public function get_value_by_key( $key, $context ) {
		...
		switch ( $context ) {
			...
			case 'special_status':
				return $this->get_special_status_text_by_slug( $key );
		}
	 ...
	 * @since 0.8.2
	 */
	public function get_import_options(): array {
		$options = [];
		...
		// Countries
		$options['countries'] = [];
		foreach ( $this->cb_get_countries() as $country_code => $country_title ) {
			$options['countries'][] = [
				'id'   => $country_code,
				'name' => $country_title,
			];
		...
		// Teams
		$options['clubs'] = [];
		foreach ( $this->plugin->club->get_clubs_options() as $team_id => $team_name ) {
			$options['clubs'][] = [
				'id'   => $team_id,
				'name' => $team_name,
			];
	...
	/**
	 * Get list of available game special statuses
	 *
	 * @params array $ids
	 *
	 * @return array
	 * @since  0.14.6
	 */
	public function get_game_special_statuses() {

		$statuses = [
			'PST'  => __( 'Match Postponed', 'anwp-football-leagues' ),
			'TBD'  => __( 'Time To Be Defined', 'anwp-football-leagues' ),
			'CANC' => __( 'Match Cancelled', 'anwp-football-leagues' ),
		];

		return apply_filters( 'anwpfl/config/game_special_statuses', $statuses );
	}

	/**
	 * Get special game status text by its slug.
	 *
	 * @param $slug
	 *
	 * @return string
	...
	private function get_special_status_text_by_slug( $slug ) {
		...
		$available_statuses = [
			'PST'  => AnWPFL_Text::get_value( 'match__match__match_postponed', __( 'Match Postponed', 'anwp-football-leagues' ) ),
			'CANC' => AnWPFL_Text::get_value( 'match__match__match_cancelled', __( 'Match Cancelled', 'anwp-football-leagues' ) ),
		];

		return empty( $available_statuses[ $slug ] ) ? '' : $available_statuses[ $slug ];
	}

	public function get_available_circle_flags() {
		return [
			'--europe',
			'--world',
			'-england',
